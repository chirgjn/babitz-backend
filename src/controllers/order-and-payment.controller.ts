...
export class OrderAndPaymentController {
  ...
  async placeOrder(
    ...
    if (userRecord) {
      ...
      if (!restaurantExists)
        throw new HttpErrors.NotFound("restaurant doesn't exist");
      ...
      if (userCart && restaurantExists) {
        ...
        const razorpayOrder = await this.razorpayService.createOrder(newOrder);
  ...
  async acceptOrRejectOrder(
    ...
    if (restaurant) {
      const orderFilter: Where = {
        where: {restaurantId: restaurant.id, orderId: orderId},
      };
      ...
      if (
        orderRecord &&
        ['accepted', 'rejected', 'complete', 'pending'].includes(decision)
      ) {
  ...
  async rejectOrder(
    ...
    const order = await this.acceptOrRejectOrder(
      authorization,
      'rejected',
      orderId,
    );
    await this.orderHistoryRepository.create(order);
    return order;
  }
